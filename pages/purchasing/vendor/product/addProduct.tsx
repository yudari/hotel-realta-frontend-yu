...
import { doAddProdVendor } from "@/redux/purchasing/action/prodVendorActionReducer";
...
export default function AddProduct(props: any) {
  ...
  console.log("stockNametest", stock);
  ...
  const [stocks, setStocks] = useState<StockAdd[]>([]);
  ...
  //       const data = await getStocksVendor();
  //       setStocks(data);
  //     };
  //     fetchStock();
  //   }, []);
  ...
  const handleSave = async (data: FormValues) => {
    try {
      ...
      console.log("inputan:", dataAll);
      await dispatch(doAddProdVendor(dataAll));
      props.closeModal();
      console.log(doAddProdVendor);
    } catch (error) {
      console.error(error);
    }
  };
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  <Dialog.Title className="text-lg font-medium leading-6 text-gray-900">Add Product</Dialog.Title>
                  <div className="mt-4">
                    <form onSubmit={handleSubmit(handleSave, handleError)}>
                      ...
                      <div className="mb-4">
                        ...
                        <input className="shadow appearance-none border rounded w-full py-3 px-1 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="name" type="text" {...register("vepro_qty_stocked", registerOptions.vepro_qty_stocked)} />
                      ...
                      <div className="mb-4">
                        ...
                        <input className="shadow appearance-none border rounded w-full py-3 px-1 text-gray-700 leading-tight focus:outline-none focus:shadow-outline " id="price" type="text" {...register("vepro_qty_remaining", registerOptions.vepro_qty_remaining)} />
                        {errors?.vepro_qty_remaining && <p className="text-red-500 text-xs italic">{errors.vepro_qty_remaining.message}</p>}
                      </div>

                      <div className="mb-4">
                        <label className="block text-gray-700 font-bold mb-2"> Price</label>
                        <input className="shadow appearance-none border rounded w-full py-3 px-1 text-gray-700 leading-tight focus:outline-none focus:shadow-outline " id="price" type="text" {...register("vepro_price", registerOptions.vepro_price)} />
